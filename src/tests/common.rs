...
pub(super) struct DebugMidiConnection {
    inner: Rc<RefCell<DebugMidiConnectionInner>>,
    start_timestamp: Instant,
}

impl DebugMidiConnection {
    pub(super) fn new(inner: Rc<RefCell<DebugMidiConnectionInner>>) -> Self {
        Self {
            inner,
    ...
    fn print_elapsed(&self, message: &str) {
        println!(
            "[{}]\t{message}",
            self.start_timestamp.elapsed().as_secs_f32()
        )
    }
}
